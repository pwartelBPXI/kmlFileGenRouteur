...
def kmlMultiBoats(N,nbBoats):  
    ...
    print('')                                    # Leaves an empty line for clarity
    for i in range(0,nbBoats,1):
        # lonBoat = lonLatInput('lon',i)         # Uncomment the following i you want the user to enter coords in Decimal Degrees 
        # latBoat = lonLatInput('lat',i)
        lonBoat = decodeLonLatUserInput('Lon',i) # Uncomment the following i you want the user to enter coords in Degrees Minutes 
        latBoat = decodeLonLatUserInput('Lat',i)
        print('')                                # Leaves an empty line for clarity
        boatCircles = []
        for i in range(len(listRadius)):
...
# If Windy ever gets a drastic update, the html code will no longer be the same 
# This function will therefore need to be changed 
# The href will need updating 
...
def seleniumWindy(browser,reset,autoOrManual,sleepTime):
    ...
    kmlDirectory = kmlDirectory[:2] + "\\" + kmlDirectory[2:] + "\\MyPolygon.kml"  # Create the appropriate path that Selenium can read the kml file 
    ###
    #--------------------------- SUPER IMPORTANT FINISH --------------------------------#
    ...
    ######### Manual #########
    ######### 
    if (reset == 2 and autoOrManual == 'M' and sleepTime != 0): 
        print('-------------------------------------------')
        ...
        while True: 
            ...
            if resetPrompt == "y":                    # Refresh and fetch the update GPS coord once more 
                x = 2
                break
            if resetPrompt == "init":                 # ReConfig User or GPS Mode
                x = 0
                break
    ...
    ######### Manual for when you have user input #########
    ######### 
    elif (reset == 2 and autoOrManual == 'M' and sleepTime == 0): 
        print('-------------------------------------------')
        print('you are here')
        ...
        while True: 
            if resetPrompt != "init":
                resetPrompt = str(input("Type Ctrl + C or init:"))
            if resetPrompt == "init":                 # ReConfig User or GPS Mode
                x = 0
                break
        browser.get('https://www.windy.com/uploader') # Open page to Upload new kml file 
...
#------------------------- Decode Longitude and Latitude from User Input (DDMM.mm to dd) -------------------------------#


def decodeLonLatUserInput(stringLonOrLat,i):                        # Decode lon and lat from User Input
                                                                    # User Input: -77 30.5 (Degrees Minutes)
                                                                    # Output:     -77.5083 (Degrees Decimal)
                                                                    # Function will be called in kmlMultiBoats(....)

    while True:
        lonOrLat = input("Boat %d %s (Degrees Minutes): " %(i+1,stringLonOrLat))
        try:
            val = str(lonOrLat)
            deg, minutes =  re.split('[" "]', val)                  # Find the blank space between the Degrees and Minutes 
            deg, minutes = float(deg), float(minutes)           
            if deg < 0:                                             # If negative we are in the South (Lat) or West (Lon) Config 
                sign = -1
            elif deg > 0:                                           # If positive we are in the North (Lat) or East (Lon) Config 
                sign = 1
            DecDeg = sign*(abs(deg) + minutes/60)                   # Return a float 
            DecDegString = "{:.4f}".format(DecDeg)                  # Return a string of the float above 
            print("Boat %d %s (Degrees Decimal): %s" %(i+1,stringLonOrLat,DecDegString))
            # print(type(DecDeg))
            break
        except ValueError:
            print("Invalid Syntax, Try Again")                      # The syntax must feature a space between the Degrees and minutes 
    return DecDeg
...
def lonLatInput(stringLonOrLat,i):                                             # Same function as decodeLonLatUserInput(...)
                                                                               # Except we enter the values as decimal degrees
                                                                               # There is no need to convert them
                                                                               # This function is currently not in use 
                                                                               # Function will be called in kmlMultiBoats(....)

    while True:
        lonOrLat = input("Boat_%d %s: (Degrees Decimal)" %(i,stringLonOrLat))
        try:
            val = int(lonOrLat)
